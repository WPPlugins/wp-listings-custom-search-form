...
/*
	...
	Description: It is an add-on of IMPress Listings plugin which allow to create custom search widget for real estate listing management system. Designed to work with any theme using built-in templates.
	Author: Anil Meena
...
class WP_Listings_Custom_Search_Form_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'listings-custom-search-form', __( 'WP Listings - Custom Search Form', 'wp-listings-custom-search-form' ), $widget_ops, $control_ops );
	}
...
function wlcsw_plugin_activate(){
    ...
    if ( ! is_plugin_active( 'wp-listings/plugin.php' ) and current_user_can( 'activate_plugins' ) ) {
        ...
        //wp_die('Sorry, but this plugin requires the <a href="//wordpress.org/plugins/wp-listings">IMPress Listings (WP Listings)</a> Plugin to be installed and active. <br><a href="' . admin_url( 'plugins.php' ) . '">&laquo; Return to Plugins</a>');
...
function wlcsw_plugin_notice(){
    _e('<div class="error"><p>Sorry, but WP Listings Custom Search Widget Plugin requires the IMPress Listings (WP Listings) to be installed and active.</p></div>', 'wp-listings-custom-search-form');
}
...
function wlcsw_add_inline_widget_option( $widget, $return, $instance ) {
    ...
    // Are we dealing with a WP Listings custom search form widget?
